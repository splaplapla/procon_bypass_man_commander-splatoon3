...
RSpec.describe ProconBypassManCommander::Splatoon3::EnemyTargetDetectorHD do
  before(:all) do
  ...
  describe 'detect?' do
    context 'when no targets' do
      ...
      raise('!!!!!!!!') if Dir.glob('./spec/files/hd/no_targets/*.png').empty?
      Dir.glob('./spec/files/hd/no_targets/*.png') do |path|
    ...
    context 'when has targets' do
      ...
      Dir.glob('./spec/files/hd/has_targets/shooter/*.png') do |path|
        next if path.end_with?('result.png')
